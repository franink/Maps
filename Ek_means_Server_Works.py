...
def k_means(X, n_clusters, init='k-means++', precompute_distances=True,
            ...
            tol=1e-4, random_state=None, copy_x=True, n_jobs=1, k=None, par=1):
    ...
    par: int
        Free parameter for the ratio of people per cluster to an even distribution
        of people in clusters used to modify the distance function and get
        clusters of even size. Defaults to 1.
    ...
    if n_jobs == 1:
        ...
        for it in range(n_init):
            ...
            labels, inertia, centers = _kmeans_single(
                ...
                x_squared_norms=x_squared_norms, random_state=random_state, par=par)
        ...
        results = Parallel(n_jobs=n_jobs, verbose=0)(
            delayed(_kmeans_single)(X, n_clusters, max_iter=max_iter,
                ...
                precompute_distances=precompute_distances, par=par,
...
def _kmeans_single(X, n_clusters, par, max_iter=300, init='k-means++',
    ...
    people = float(X.shape[0])
    even_cluster = people/n_clusters
    ctr = 0.0 #New line 5-22
    peoplePerCluster = np.zeros(n_clusters) #Moved this from inside loop to here 5-22
    # iterations
    for i in range(max_iter):
        clus_ratio = np.ones((n_clusters,1))
        peoplePerCluster_new = np.zeros(n_clusters) # added _new 5-22
        centers_old = centers.copy()
        if labels is None:
            labels_old = None ### This was changed from None 8-22-14
        else:
            labels_old = labels.copy()  ## This was before best_labels.copy()
        if labels_old is not None:
            peoplePerCluster_new = np.bincount(labels_old, minlength = n_clusters) + 1
            peoplePerCluster = ((peoplePerCluster*(ctr-1)) + peoplePerCluster_new)/ctr #added 5-22
            clus_ratio = peoplePerCluster/even_cluster
            clus_ratio.resize(n_clusters,1)
        # labels assignement is also called the E-step of EM
        #print 'ctr, people_old, people_new', ctr, peoplePerCluster, peoplePerCluster_new
        labels, inertia = \
                _labels_inertia(X, x_squared_norms, centers,
                                precompute_distances=precompute_distances,
                                distances=distances, clus_ratio=clus_ratio, par=par)

        # computation of the means is also called the M-step of EM
        centers = _centers(X, labels, n_clusters, distances)
        ctr += 1 # added 5-22 counter for running average of population count

        if verbose:
            print 'Iteration %i, inertia %s, best_inertia %s' % (i, inertia, best_inertia)
            
        #### This section was added 8-22-14 to only pick among legal solutions (even clusters)
        pplPerClusterFinal = np.bincount(labels)
        #print 'pplPerClusterfinal ', pplPerClusterFinal
        ClusMax = max(pplPerClusterFinal)
        ClusMin = min(pplPerClusterFinal)
        ...
        ### This is repeated to store the labels and centers so the algorithm keeps learning but inertia is not stored
#        else:                
#            if best_inertia is None or inertia < best_inertia:
#                    best_labels = labels.copy()
#                    best_centers = centers.copy()
#                    #best_inertia    This is commented out 8-22-14
############ I ended commenting this section because it wasn't working new option is including labels_old up top
...
def _labels_inertia_precompute_dense(X, x_squared_norms, centers, clus_ratio, par):
    n_samples = X.shape[0]
    k = centers.shape[0]
    distances = euclidean_distances(centers, X, x_squared_norms,
                                    squared=True)
    dist_old = distances
    distances = distances * (clus_ratio**par)
    #print 'distances sin ratio, con ratio', np.sum(dist_old), np.sum(distances)
    #print 'clus_ratio', clus_ratio
...
def _labels_inertia(X, x_squared_norms, centers, clus_ratio, par,
...
class KMeans(BaseEstimator):
    ...
    def __init__(self, n_clusters=8, init='k-means++', n_init=10, max_iter=300,
                 ...
                 verbose=0, random_state=None, copy_x=True, n_jobs=1, k=None, par=1):
    ...
    def fit(self, X, y=None):
        ...
        self.cluster_centers_, self.labels_, self.inertia_ = k_means(
            ...
            n_jobs=self.n_jobs, par=self.par)
        return self
